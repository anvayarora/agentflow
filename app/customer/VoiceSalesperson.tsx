...
export default function VoiceSalesperson({ initialSessionId }: { initialSessionId?: string | null }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void fetch("/api/salespeople").then((response) => response.json() as Promise<{ profiles?: Profile[] }>).then(async (body) => { const next = body.profiles || []; const defaultProfile = next.find((profile) => profile.isMerchantDefault) || next[0] || null; setProfiles(next); setSelected(defaultProfile); if (!sessionId && defaultProfile) { const sessionResponse = await fetch("/api/voice/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ salespersonProfileId: defaultProfile.id, language, voiceEnabled: false, selectorOpened: true }) }); const sessionBody = await sessionResponse.json() as { sessionId?: string }; if (sessionResponse.ok && sessionBody.sessionId) setSessionId(sessionBody.sessionId); } }).catch(() => setMessage("Salesperson selector is unavailable.")); }, 0); return () => window.clearTimeout(timer); }, [sessionId, language]);

  const ensureSession = async () => {
    if (sessionId) return sessionId;
    const response = await fetch("/api/voice/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ salespersonProfileId: selected?.id, language, voiceEnabled: voiceOn }) });
  ...
  const sendTurn = async (value: string) => {
    const request = value.trim(); if (!request) return;
    setTranscript(request); setText(""); setState("THINKING"); setMessage("");
    try { const currentSession = await ensureSession(); const response = await fetch("/api/voice/turn", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: currentSession, salespersonProfileId: selected?.id, language, voiceEnabled: voiceOn, message: request, storefrontContext: { pageType: "home", url: window.location.href } }) }); const body = await response.json() as { message?: string; voice?: { audioBase64?: string; mimeType?: string; error?: string }; error?: string }; if (!response.ok && !body.message) throw new Error(body.error || "The salesperson is unavailable."); const reply = body.message || "I can keep helping by text."; setLastReply(reply); if (body.voice?.audioBase64 && voiceOn) { setState("SPEAKING"); audio.current?.pause(); audio.current = new Audio(`data:${body.voice.mimeType || "audio/wav"};base64,${body.voice.audioBase64}`); audio.current.onended = () => setState("WAITING_FOR_SHOPPER"); await audio.current.play(); } else setState("WAITING_FOR_SHOPPER"); applySemanticAction(request); } catch (error) { setState("ERROR"); setMessage(error instanceof Error ? error.message : "The salesperson is unavailable."); }
  };
...
function applySemanticAction(text: string) {
