...
export async function GET(request: Request) {
  ...
  } catch (error) { return Response.json(shopifyPublicError(error, "Your saved list could not be loaded."), { status: 400 }); }
}
...
export async function POST(request: Request) {
  try {
    ...
    const products = (await Promise.all(shortlist.productIds.map((productId) => getProduct(context, session, productId)))).filter(Boolean);
    return Response.json({ sessionId: session.id, productIds: shortlist.productIds, products });
  } catch (error) { return Response.json(shopifyPublicError(error, "Your saved list could not be updated."), { status: 400 }); }
}
