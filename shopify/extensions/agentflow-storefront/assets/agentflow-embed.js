(function () {
  ...
  var discardNextRecording = false;
  ...
  function stopVoiceCapture() {
    ...
    if (recorder) discardNextRecording = true;
    stopRecording();
    if (voiceAudio) { try { voiceAudio.pause(); voiceAudio.currentTime = 0; } catch { /* already stopped */ } }
  ...
  function unlockAudioPlayback() {
    ...
    } catch { voiceAudio = null; }
  }
  function playAudio(audioBase64, mimeType) { if (!audioBase64 || voiceMuted) return; try { if (!voiceAudio) unlockAudioPlayback(); var audio = voiceAudio || new Audio(); audio.pause(); audio.src = "data:" + (mimeType || "audio/wav") + ";base64," + audioBase64; setVoiceUiState("speaking", selectedGuide().displayName + " is speaking", "Take your time — I’m here when you’re ready."); audio.onended = function () { if (voiceMode) setVoiceUiState("idle", "Ready when you are", "Tap Start talking for your next request."); }; audio.play().catch(function () { setVoiceStatus("Tap Start talking, then try again to hear your guide."); }); } catch { setVoiceStatus("Voice output is temporarily unavailable."); } }
  ...
  async function startRecording() { if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia || !window.MediaRecorder) { setVoiceUiState("error", "Microphone unavailable", "You can continue with chat in this browser."); setVoiceStatus("This browser does not support microphone input. You can still type."); return; } try { if (!voiceSession) await ensureVoiceSession(false); recorderStream = await navigator.mediaDevices.getUserMedia({ audio: true }); var mimeType = MediaRecorder.isTypeSupported("audio/webm;codecs=opus") ? "audio/webm;codecs=opus" : "audio/webm"; var chunks = []; discardNextRecording = false; recorder = new MediaRecorder(recorderStream, { mimeType: mimeType }); recorder.ondataavailable = function (event) { if (event.data && event.data.size) chunks.push(event.data); }; recorder.onstop = function () { var blob = new Blob(chunks, { type: mimeType }); var discard = discardNextRecording; discardNextRecording = false; if (recorderStream) recorderStream.getTracks().forEach(function (track) { track.stop(); }); recorder = null; recorderStream = null; if (recorderTimer) window.clearTimeout(recorderTimer); recorderTimer = null; micButton.classList.remove("is-recording"); voiceToggle.setAttribute("aria-pressed", "false"); voiceToggle.classList.remove("is-active"); voiceToggle.querySelector("span:last-child").textContent = "Start talking"; if (!discard && blob.size > 0) transcribeRecording(blob); }; recorder.start(); micButton.classList.add("is-recording"); voiceToggle.setAttribute("aria-pressed", "true"); voiceToggle.classList.add("is-active"); voiceToggle.querySelector("span:last-child").textContent = "Stop & send"; setVoiceUiState("listening", selectedGuide().displayName + " is listening…", "Tap Stop & send when you’re finished."); setVoiceStatus("Listening…"); recorderTimer = window.setTimeout(stopRecording, 12000); } catch (error) { setVoiceUiState("error", "Microphone permission needed", "Allow microphone access, or continue by typing."); setVoiceStatus(error && error.message ? error.message : "Microphone permission is needed for voice input."); } }
  ...
  async function toggleVoiceMode() { if (recorder) { stopRecording(); return; } unlockAudioPlayback(); try { if (!voiceSession) await ensureVoiceSession(true); voiceMode = true; transition(PRESENTATION.VOICE_ACTIVE, { focus: false }); await startRecording(); } catch (error) { setVoiceUiState("error", "Voice is taking a moment", "You can continue with chat while we reconnect."); setVoiceStatus(error && error.message ? error.message : "Voice mode is temporarily unavailable."); } }
